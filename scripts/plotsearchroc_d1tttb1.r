...
# Alex Stivala
# July 2008
# $Id: plotsearchroc_d1tttb1.r 1946 2008-10-04 05:14:43Z astivala $
...
qp_numeric <- read.table('d1tttb1.tsrchn.ff.rtab',header=TRUE)
qp_numeric_penalizessetype<-read.table('d1tttb1.tsrchn.tf.rtab',header=TRUE)
qp_numeric_ordering_penalizessetype <- read.table('d1tttb1.tsrchn.tt.rtab', header=TRUE)

qp_discrete <- read.table('d1tttb1.tsrchd.ff.rtab',header=TRUE)
qp_discrete_penalizessetype<-read.table('d1tttb1.tsrchd.tf.rtab',header=TRUE)
qp_discrete_ordering_penalizessetype <- read.table('d1tttb1.tsrchd.tt.rtab', header=TRUE)


# EPS suitable for inserting into LaTeX
postscript('searchroc_d1tttb1.eps',onefile=FALSE,paper="special",horizontal=FALSE, 
           width = 9, height = 6)

plot(c(0,1),c(0,1),type='l',xlim=c(0,1),ylim=c(0,1),xlab="False Positive Rate",ylab="True Positive Rate",lty=3,main='ROC curves for query d1tttb1 against ASTRAL 95% seq id SCOP as truth')
